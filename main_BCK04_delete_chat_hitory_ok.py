...
def delete_chat_history(user_id):
    """ลบประวัติการสนทนาของ user ออกจาก Azure Cognitive Search"""
    try:
        # 🔹 ดึงเอกสารทั้งหมดของ user_id
        results = chat_history_client.search(
            search_text="*",
            filter=f"user_id eq '{user_id}'"
        )
        
        # 🔹 รวบรวมรายการ ID ของเอกสารที่ต้องการลบ
        documents_to_delete = [{"id": result["id"]} for result in results]

        if documents_to_delete:
            chat_history_client.delete_documents(documents=documents_to_delete)
...
def search_chat_history(user_id, query, top=2):
    try:
        results = chat_history_client.search(
